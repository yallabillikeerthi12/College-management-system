...
{
    header("Location: login1_res.php");
    exit();
}
...
{
    $clearQuery = $con->query("TRUNCATE TABLE result");

    if(!$clearQuery)
    {
        die("Clear Failed : " . $con->error);
    }
}
...
if(isset($_POST['add']))
{
    ...
    $sub1_name  = trim($_POST['sub1_name']);
    $sub2_name  = trim($_POST['sub2_name']);
    $sub3_name  = trim($_POST['sub3_name']);
    $sub4_name  = trim($_POST['sub4_name']);
    $sub5_name  = trim($_POST['sub5_name']);
...
<a href="logout1_res.php" class="logout-btn">Logout</a>
</div>
...
$result_data = $con->query("SELECT * FROM result");
